...
const Coffee_beans_func = ({ item,onPress }) => {
  ...
  return (
    ...
    <LinearGradient style={{borderRadius:30,paddingBottom:'12%',height:'98%',width:'105%'}} colors={['#262B33', '#0C0F14']} start={{x:0,y:0}} end={{x:1.5,y:0}}>

      <Image
        source={{ uri: item.imgUrl }}
        style={[styles.image]}
      />
      <View style={{position:'absolute',width:'31.4%',height:22,backgroundColor:'rgba(0, 0, 0, 0.6)',left:'60%',bottom:'95.1%',borderBottomLeftRadius:30,borderTopRightRadius:30,alignItems:'center',justifyContent:'center'}}>
      ...
      <View style={{width:'100%',paddingHorizontal:10,flexDirection:'row',alignItems:'center'}}>
        ...
        onPress={()=>{
          ...
          if(item_array[itemIndex]&&itemIndex!==-1&&cartData.length>0){
             ...
             addValueToCart({id:item.id,name:item.title,small:1,medium:0,large:0,img:item.imgUrl,small_price:item.small_p,medium_price:item.medium_p,large_price:item.large_p,total_p:item.small_p,smallSize:item.size_C_S,mediumSize:item.size_C_M,largeSize:item.size_C_L})
          }
